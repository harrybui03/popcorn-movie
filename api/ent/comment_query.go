...
func (cq *CommentQuery) QueryMovie() *MovieQuery {
	query := (&MovieClient{config: cq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := cq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (cq *CommentQuery) WithMovie(opts ...func(*MovieQuery)) *CommentQuery {
	query := (&MovieClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if query := cq.withMovie; query != nil {
		if err := cq.loadMovie(ctx, query, nodes, nil,
			func(n *Comment, e *Movie) { n.Edges.Movie = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CommentQuery) loadMovie(ctx context.Context, query *MovieQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Movie)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Comment)
	for i := range nodes {
		fk := nodes[i].MovieID
...
func (cq *CommentQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := cq.ctx.Fields; len(fields) > 0 {
		...
		if cq.withMovie != nil {
			_spec.Node.AddColumnOnce(comment.FieldMovieID)
		}
	}
