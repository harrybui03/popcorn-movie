...
func (sq *SeatQuery) QueryRoom() *RoomQuery {
	query := (&RoomClient{config: sq.config}).Query()
...
func (sq *SeatQuery) QueryTickets() *TicketQuery {
	query := (&TicketClient{config: sq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := sq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (sq *SeatQuery) Clone() *SeatQuery {
	...
	return &SeatQuery{
		...
		withTickets: sq.withTickets.Clone(),
...
func (sq *SeatQuery) WithRoom(opts ...func(*RoomQuery)) *SeatQuery {
	query := (&RoomClient{config: sq.config}).Query()
...
func (sq *SeatQuery) WithTickets(opts ...func(*TicketQuery)) *SeatQuery {
	query := (&TicketClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withTickets = query
	return sq
}
...
func (sq *SeatQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Seat, error) {
	...
	if query := sq.withRoom; query != nil {
		if err := sq.loadRoom(ctx, query, nodes, nil,
			func(n *Seat, e *Room) { n.Edges.Room = e }); err != nil {
			return nil, err
		}
	}
	if query := sq.withTickets; query != nil {
		if err := sq.loadTickets(ctx, query, nodes,
			func(n *Seat) { n.Edges.Tickets = []*Ticket{} },
			func(n *Seat, e *Ticket) { n.Edges.Tickets = append(n.Edges.Tickets, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SeatQuery) loadRoom(ctx context.Context, query *RoomQuery, nodes []*Seat, init func(*Seat), assign func(*Seat, *Room)) error {
...
func (sq *SeatQuery) loadTickets(ctx context.Context, query *TicketQuery, nodes []*Seat, init func(*Seat), assign func(*Seat, *Ticket)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(ticket.FieldSeatID)
	}
	query.Where(predicate.Ticket(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(seat.TicketsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (sq *SeatQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := sq.ctx.Fields; len(fields) > 0 {
		...
		if sq.withRoom != nil {
			_spec.Node.AddColumnOnce(seat.FieldRoomID)
		}
	}
...
func (sq *SeatQuery) ForUpdate(opts ...sql.LockOption) *SeatQuery {
...
func (sq *SeatQuery) ForShare(opts ...sql.LockOption) *SeatQuery {
	if sq.driver.Dialect() == dialect.Postgres {
		sq.Unique(false)
	}
	sq.modifiers = append(sq.modifiers, func(s *sql.Selector) {
