...
func (fq *FoodQuery) QueryFoodOrderLines() *FoodOrderLineQuery {
	query := (&FoodOrderLineClient{config: fq.config}).Query()
...
func (fq *FoodQuery) Clone() *FoodQuery {
	...
	return &FoodQuery{
		...
		withFoodOrderLines: fq.withFoodOrderLines.Clone(),
...
func (fq *FoodQuery) WithFoodOrderLines(opts ...func(*FoodOrderLineQuery)) *FoodQuery {
	query := (&FoodOrderLineClient{config: fq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	fq.withFoodOrderLines = query
	return fq
}
...
func (fq *FoodQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Food, error) {
	...
	if query := fq.withFoodOrderLines; query != nil {
		if err := fq.loadFoodOrderLines(ctx, query, nodes,
			func(n *Food) { n.Edges.FoodOrderLines = []*FoodOrderLine{} },
			func(n *Food, e *FoodOrderLine) { n.Edges.FoodOrderLines = append(n.Edges.FoodOrderLines, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (fq *FoodQuery) loadFoodOrderLines(ctx context.Context, query *FoodOrderLineQuery, nodes []*Food, init func(*Food), assign func(*Food, *FoodOrderLine)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(foodorderline.FieldFoodID)
	}
	query.Where(predicate.FoodOrderLine(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(food.FoodOrderLinesColumn), fks...))
	}))
...
func (fq *FoodQuery) ForUpdate(opts ...sql.LockOption) *FoodQuery {
...
func (fq *FoodQuery) ForShare(opts ...sql.LockOption) *FoodQuery {
	if fq.driver.Dialect() == dialect.Postgres {
		fq.Unique(false)
	}
	fq.modifiers = append(fq.modifiers, func(s *sql.Selector) {
