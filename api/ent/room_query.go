...
// QueryTheater chains the current query on the "theater" edge.
func (rq *RoomQuery) QueryTheater() *TheaterQuery {
	query := (&TheaterClient{config: rq.config}).Query()
...
// WithTheater tells the query-builder to eager-load the nodes that are connected to
// the "theater" edge. The optional arguments are used to configure the query builder of the edge.
func (rq *RoomQuery) WithTheater(opts ...func(*TheaterQuery)) *RoomQuery {
	query := (&TheaterClient{config: rq.config}).Query()
...
func (rq *RoomQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Room, error) {
	...
	if query := rq.withTheater; query != nil {
		if err := rq.loadTheater(ctx, query, nodes, nil,
			func(n *Room, e *Theater) { n.Edges.Theater = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (rq *RoomQuery) loadTheater(ctx context.Context, query *TheaterQuery, nodes []*Room, init func(*Room), assign func(*Room, *Theater)) error {
