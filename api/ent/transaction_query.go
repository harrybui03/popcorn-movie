...
type TransactionQuery struct {
	...
	withFoodOrderLines *FoodOrderLineQuery
...
func (tq *TransactionQuery) QueryTickets() *TicketQuery {
...
// QueryFoodOrderLines chains the current query on the "food_order_lines" edge.
func (tq *TransactionQuery) QueryFoodOrderLines() *FoodOrderLineQuery {
	query := (&FoodOrderLineClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TransactionQuery) Clone() *TransactionQuery {
	...
	return &TransactionQuery{
		...
		withFoodOrderLines: tq.withFoodOrderLines.Clone(),
...
func (tq *TransactionQuery) WithTickets(opts ...func(*TicketQuery)) *TransactionQuery {
	query := (&TicketClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithFoodOrderLines tells the query-builder to eager-load the nodes that are connected to
// the "food_order_lines" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TransactionQuery) WithFoodOrderLines(opts ...func(*FoodOrderLineQuery)) *TransactionQuery {
	query := (&FoodOrderLineClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withFoodOrderLines = query
	return tq
}
...
//		GroupBy(transaction.FieldTotal).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (tq *TransactionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Transaction, error) {
	...
	if query := tq.withTickets; query != nil {
		if err := tq.loadTickets(ctx, query, nodes,
			func(n *Transaction) { n.Edges.Tickets = []*Ticket{} },
			func(n *Transaction, e *Ticket) { n.Edges.Tickets = append(n.Edges.Tickets, e) }); err != nil {
			return nil, err
		}
	}
	if query := tq.withFoodOrderLines; query != nil {
		if err := tq.loadFoodOrderLines(ctx, query, nodes,
			func(n *Transaction) { n.Edges.FoodOrderLines = []*FoodOrderLine{} },
			func(n *Transaction, e *FoodOrderLine) { n.Edges.FoodOrderLines = append(n.Edges.FoodOrderLines, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TransactionQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *User)) error {
...
func (tq *TransactionQuery) loadTickets(ctx context.Context, query *TicketQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Ticket)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(ticket.FieldTransactionID)
	}
	query.Where(predicate.Ticket(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(transaction.TicketsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (tq *TransactionQuery) loadFoodOrderLines(ctx context.Context, query *FoodOrderLineQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *FoodOrderLine)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Transaction)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(foodorderline.FieldTransactionID)
	}
	query.Where(predicate.FoodOrderLine(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(transaction.FoodOrderLinesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
