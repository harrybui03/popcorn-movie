...
func (tq *TicketQuery) QueryTransaction() *TransactionQuery {
...
func (tq *TicketQuery) QuerySeat() *SeatQuery {
	query := (&SeatClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryShowTime chains the current query on the "show_time" edge.
func (tq *TicketQuery) QueryShowTime() *ShowTimeQuery {
	query := (&ShowTimeClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TicketQuery) Clone() *TicketQuery {
	...
	return &TicketQuery{
		...
		withSeat:        tq.withSeat.Clone(),
		withShowTime:    tq.withShowTime.Clone(),
...
func (tq *TicketQuery) WithTransaction(opts ...func(*TransactionQuery)) *TicketQuery {
	query := (&TransactionClient{config: tq.config}).Query()
...
func (tq *TicketQuery) WithSeat(opts ...func(*SeatQuery)) *TicketQuery {
	query := (&SeatClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// WithShowTime tells the query-builder to eager-load the nodes that are connected to
// the "show_time" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TicketQuery) WithShowTime(opts ...func(*ShowTimeQuery)) *TicketQuery {
	query := (&ShowTimeClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withShowTime = query
	return tq
}
...
func (tq *TicketQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Ticket, error) {
	...
	if query := tq.withTransaction; query != nil {
		if err := tq.loadTransaction(ctx, query, nodes, nil,
			func(n *Ticket, e *Transaction) { n.Edges.Transaction = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withSeat; query != nil {
		if err := tq.loadSeat(ctx, query, nodes, nil,
			func(n *Ticket, e *Seat) { n.Edges.Seat = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withShowTime; query != nil {
		if err := tq.loadShowTime(ctx, query, nodes, nil,
			func(n *Ticket, e *ShowTime) { n.Edges.ShowTime = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TicketQuery) loadTransaction(ctx context.Context, query *TransactionQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *Transaction)) error {
...
func (tq *TicketQuery) loadSeat(ctx context.Context, query *SeatQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *Seat)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Ticket)
	for i := range nodes {
		fk := nodes[i].SeatID
...
func (tq *TicketQuery) loadShowTime(ctx context.Context, query *ShowTimeQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *ShowTime)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Ticket)
	for i := range nodes {
		fk := nodes[i].ShowTimeID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "show_time_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TicketQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := tq.ctx.Fields; len(fields) > 0 {
		...
		if tq.withTransaction != nil {
			_spec.Node.AddColumnOnce(ticket.FieldTransactionID)
		}
		if tq.withSeat != nil {
			_spec.Node.AddColumnOnce(ticket.FieldSeatID)
		}
		if tq.withShowTime != nil {
			_spec.Node.AddColumnOnce(ticket.FieldShowTimeID)
		}
	}
